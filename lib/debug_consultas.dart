...
Stream<List<AppointmentModel>> debugGetTodayProfessionalAppointmentsStream(
  ...
  final endOfTomorrow = DateTime.utc(now.year, now.month, now.day + 1, 23, 59, 59);
  ...
  try {
    ...
    final snapshot1 = await firestore
        ...
        .where('dateTime', isGreaterThanOrEqualTo: Timestamp.fromDate(startOfDayUTC))
    ...
    final snapshot2 = await firestore
        .collection('appointments')
        .where('professionalId', isEqualTo: professionalId)
        .where('dateTime', isGreaterThanOrEqualTo: Timestamp.fromDate(startOfDayLocal))
        .where('dateTime', isLessThanOrEqualTo: Timestamp.fromDate(endOfDayLocal))
        .orderBy('dateTime')
        .get();
    ...
    final snapshot3 = await firestore
        .collection('appointments')
        .where('professionalId', isEqualTo: professionalId)
        .where('dateTime', isGreaterThanOrEqualTo: Timestamp.fromDate(startOfYesterday))
        .where('dateTime', isLessThanOrEqualTo: Timestamp.fromDate(endOfTomorrow))
        .orderBy('dateTime')
        .get();
    ...
    if (snapshot4.docs.isNotEmpty) {
      for (var doc in snapshot4.docs.take(3)) { // Mostrar apenas 3 primeiras
        final data = doc.data();
