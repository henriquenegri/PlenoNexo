...
class _ProfileEditScreenState extends State<ProfileEditScreen> {
  ...
  final List<String> _neurodiversityOptions = [
    'Autismo', 'TDAH', 'Transtorno Bipolar', 'Dislexia', 'Dispraxia', 'TPS', 'Discalculia', 'Ansiedade', 'Síndrome Tourette', 'TOC', 'Depressão', 'Nenhum', 'Outros',
  ];
  ...
  Future<void> _loadUserData() async {
    ...
    if (user != null && mounted) {
      setState(() {
        ...
        _selectedBirthDate = user.birthDate != null ? DateTime.tryParse(user.birthDate!) : null;
  ...
  Future<void> _saveChanges() async {
    ...
    if (_currentPasswordController.text.isNotEmpty || _passwordController.text.isNotEmpty) {
    ...
    try {
      ...
      if (_selectedNeurodiversities.contains('Outros') && _otherNeurodiversityController.text.trim().isNotEmpty) {
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  IconButton(icon: const Icon(Icons.arrow_back, color: Color(0xFF2A475E)), onPressed: () => Navigator.pop(context)),
                  Image.asset('assets/img/PlenoNexo.png', height: 60),
                  const SizedBox(width: 12),
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(_getFirstName(), style: GoogleFonts.montserrat(fontSize: 18, fontWeight: FontWeight.w600, color: const Color(0xFF2A475E))),
                    Text(DateTime.now().toString().split(' ')[0], style: GoogleFonts.poppins(fontSize: 14, color: const Color(0xFF2A475E).withOpacity(0.7))),
                  ]),
                  const Spacer(),
                  Container(
                    decoration: BoxDecoration(color: const Color(0xFF2A475E).withOpacity(0.9), shape: BoxShape.circle),
                    child: IconButton(icon: const Icon(Icons.notifications_outlined, color: Colors.white, size: 28), onPressed: () {}),
                  ),
                ],
              ),
            ),
            ...
            Expanded(
              child: Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFF2A475E), borderRadius: BorderRadius.circular(15.0)),
                child: SingleChildScrollView(
                  ...
                  child: Form(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Center(child: Text('Editar Perfil', style: GoogleFonts.montserrat(fontSize: 24, fontWeight: FontWeight.w600, color: Colors.white))),
                        ...
                        _buildTextField(label: 'Cidade', controller: _cityController),
                        _buildTextField(label: 'Telefone', controller: _phoneController, keyboardType: TextInputType.phone, inputFormatters: [_phoneFormatter]),

                        const SizedBox(height: 24),
                        Text('Alterar Senha', style: GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.white)),
                        const SizedBox(height: 16),
                        ...
                        _buildTextField(
                          ...
                          suffixIcon: IconButton(
                            icon: Icon(_isCurrentPasswordVisible ? Icons.visibility : Icons.visibility_off, color: Colors.grey[600]),
                          ...
                          children: [
                            Expanded(
                              child: ElevatedButton(
                                ...
                                child: Text('CANCELAR', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600)),
                              ),
                            ),
                            ...
                            Expanded(
                              child: ElevatedButton(
                                ...
                                child: _isLoading
                                    ...
                                    : Text('SALVAR ALTERAÇÕES', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600)),
                              ),
                            ),
                          ],
                        ),
