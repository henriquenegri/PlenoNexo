...
class AppointmentService {
  ...
  Future<List<AppointmentModel>> _getProfessionalAppointmentsFallback(
    String professionalId,
  ) async {
    try {
      print(
        'DEBUG: Usando fallback para buscar todas as consultas do profissional',
      );
      ...
      print(
        'DEBUG: Fallback encontrou ${snapshot.docs.length} documentos na coleção',
      );
      ...
      for (var doc in snapshot.docs) {
        ...
        if (data['professionalId'] == professionalId) {
          ...
          appointments.add(appointment);
          print(
            'DEBUG: Fallback - consulta adicionada: ${appointment.subject}',
          );
        }
      }
      ...
      print(
        'DEBUG: Fallback - total de consultas do profissional: ${appointments.length}',
      );
      return appointments;
    } catch (error) {
  ...
  Stream<List<AppointmentModel>> getTodayProfessionalAppointmentsStream(
    ...
    return _firestore
        ...
        .asyncMap((snapshot) async {
          print(
            'DEBUG: Snapshot recebido com ${snapshot.docs.length} documentos',
          );
          ...
          for (var doc in snapshot.docs) {
            final appointment = AppointmentModel.fromFirestore(doc);
            print(
              'DEBUG: Processando consulta: ${appointment.subject} às ${appointment.dateTime}',
            );
            ...
            try {
              ...
              if (userDoc.exists) {
                ...
                print(
                  'DEBUG: Paciente não encontrado para ID: ${appointment.patientId}',
                );
              }
            } catch (e) {
              print('DEBUG: Erro ao buscar paciente: $e');
          ...
          print(
            'DEBUG: Total de consultas de hoje: ${todayAppointments.length}',
          );
  ...
  Stream<List<AppointmentModel>> getTodayProfessionalAppointmentsStreamSafe(
    ...
    print(
      'DEBUG: Safe method - buscando consultas com filtro por profissional',
    );
    ...
    return _firestore
        ...
        .map((snapshot) {
          print(
            'DEBUG: Safe method - encontradas ${snapshot.docs.length} consultas',
          );
          ...
          print(
            'DEBUG: Safe method - consultas de hoje: ${todayAppointments.length}',
          );
          return todayAppointments;
        })
        .handleError((error) {
  ...
  Stream<List<AppointmentModel>> getTodayProfessionalAppointmentsFallback(
    ...
    return _firestore
        ...
        .map((snapshot) {
          print(
            'DEBUG: Fallback - total de documentos na coleção: ${snapshot.docs.length}',
          );
          ...
          final professionalAppointments = snapshot.docs
              ...
              .map((doc) {
                final appointment = AppointmentModel.fromFirestore(doc);
                print(
                  'DEBUG: Fallback - consulta encontrada: ${appointment.subject}',
                );
                return appointment;
              })
              .toList();
          ...
          print(
            'DEBUG: Fallback - consultas do profissional: ${professionalAppointments.length}',
          );

          // Filtrar por data
          final now = DateTime.now();
          final todayAppointments = professionalAppointments.where((
            appointment,
          ) {
            final appointmentDate = appointment.dateTime;
            return appointmentDate.year == now.year &&
          ...
          print(
            'DEBUG: Fallback - consultas de hoje: ${todayAppointments.length}',
          );
          return todayAppointments;
        })
        .handleError((error) {
  ...
  Stream<List<AppointmentModel>> getProfessionalAppointmentsFallback(
    ...
    print(
      'DEBUG: Usando fallback para visualizar todas as consultas do profissional',
    );
    ...
    return _firestore
        ...
        .map((snapshot) {
          print(
            'DEBUG: Visualizar todas - total de documentos na coleção: ${snapshot.docs.length}',
          );
          ...
          final professionalAppointments = snapshot.docs
              ...
              .map((doc) {
                final appointment = AppointmentModel.fromFirestore(doc);
                print(
                  'DEBUG: Visualizar todas - consulta encontrada: ${appointment.subject}',
                );
                return appointment;
              })
              .toList();
          ...
          print(
            'DEBUG: Visualizar todas - consultas do profissional: ${professionalAppointments.length}',
          );
  ...
  Stream<List<AppointmentModel>> getProfessionalAppointmentsSimple(
    ...
    return _firestore
        ...
        .asyncMap((snapshot) async {
          print(
            'DEBUG: Simples - encontradas ${snapshot.docs.length} consultas',
          );
          ...
          appointments.sort((a, b) => b.dateTime.compareTo(a.dateTime));
          print(
            'DEBUG: Processadas ${appointments.length} consultas com nomes',
          );
          return appointments;
        })
        .handleError((error) {
        ...
        .asyncMap((snapshot) async {
          ...
          for (var doc in snapshot.docs) {
            ...
            if (appointment.professionalName == null ||
                appointment.professionalName!.isEmpty) {
              try {
                ...
                if (professionalDoc.exists) {
                  final professionalModel = ProfessionalModel.fromFirestore(
                    professionalDoc,
                  );
